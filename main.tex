...
\abstract{
    ...
    Co-occurrence networks are widely used for deducing microbial associations from metagenomic data, yet spurious associations and tool dependencies impede their accuracy. 
    “Reverse ecology” utilizes genomic and metabolic modeling to predict ecological traits for less-understood microorganisms and their interactions within microbial communities. 
    \microbetag, a microbial co-occurrence network annotator, employs reverse ecology to enhance/reduce confidence in network associations.


    \textbf{Results:} 
    % the main findings
    microbetagDB consists of 34,608 high-quality Genome Taxonomy DataBase genomes, along with their corresponding KEGG-annotations and genome-scale metabolic reconstructions. Seed sets (externally required compounds) were retrieved from the latter, supporting the prediction of over 1 billion potential pairwise metabolic interactions. 
    Relying on KEGG MODULE definitions, $342,000$ unique pathway complements were exported based on pairwise overlaps between the annotated genomes. 
    Predictions for 33 phenotypic traits (e.g., pH optima, oxygen tolerance etc.) are also incorporated in microbetagDB based on annotated genomes.
    \microbetag~maps node taxonomies to their closest genome in microbetagDB and annotates them with phenotypic traits, coupled with a literature-based assessment of functional potential. 
    Edges are then annotated by screening the pairwise associations over the pathway complements and the seed complementarities.
    
    \microbetag~successfully identified potential metabolic interactions between \textit{Variovorax} and neighboring species, enabling the latter to produce thiamine in a network derived from a mining waste microbial consortium, as the authors independently verified.
    Accessible as a CytoscapeApp, \microbetag~offers a user-friendly platform called microbetag GUI (MGG), that facilitates the prompt transition from an abundance table to a fully annotated co-occurrence network, providing a quick and intuitive mean to yielding explore hypotheses about the microbial interactions. 

    \textbf{Conclusions:} 
    % a brief summary and potential implications
    microbetagDB provides insight on the range of potential complementarities and their distribution among prokaryotes and benefits eco-evolutionary studies.
