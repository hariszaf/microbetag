...
\label{sec1}
    ...
    % KF: unique repertoire of metabolites - I would rather talk about a repertoire of metabolic reactions; the metabolites are a consequence of that
    % KF: showcases adaptability -> adapts to; across various metabolic niches -> to various niche
    ...
    % Species with similar trophic or other ecological properties are able to occupy the same niche within an ecosystem, and the particular species that occupies a particular space is then determined by stochastic recruitment~\cite{burke2011bacterial}. 
    % This means that within a group of species with similar ecologies, the “lottery” for space is won by whoever gets there first
    ...
    % KF: Such enhancers (well-articulated, interesting, essential etc) are routinely removed by editors in good journals. Please don't use them, they don't add useful info.
    ...
    % KF: suggested by, not on; paired interactions -> taxon pair
    ...
    % KF: consolidates -> provides ("consolidates" makes no sense to me here)
    ...
    % KF: "Key concept on the presented approach is the exploitation of the reverse ecology approach" -> The key concept here is the reverse ecology approach
    ...
    % KF: I contest "successful proxy" because there is no data on that, so no proof. You can write "Seeds are a useful proxy" instead
    ...
    % KF: "Contrary" not correctly spelled (and I think it should be an adverb). I propose: In contrast
    ...
    % KF: "paired-wise" doesn't exist in English - do you mean pairwise?; that currently not capable of -> that they are currently not capable of
    ...
    % KF: integrates user's -> integrates a user's; 2 x integrate in that sentence, perhaps you can replace the first with "augments" and for the second write: by integrating
...
\label{sec:implementation}
    ...
    \label{subsec:taxonomies}
        ...
        % KF: children taxa -> child taxa (see plural of compound nouns in English)
    ...
    \label{subsec:preprocess}
        ...
        % KF: in order to faciliate the user -> to help the user
        Likewise, when the abundance table consists of more than $1,000$ taxa, providing a network as an input is mandatory. Again, in order to faciliate the user, \microbetag preprocess Docker image supports the inference of a network using FlashWeave. 


    \subsection*{Literature based nodes annotation }
    \label{subsec:fapro}

        % KF: Tara Oceans samples -> Tara Ocean samples; "announcements of cultured representatives"? do you mean descriptions of cultured representatives?
    ...
    \label{subsec:path-compl}
        ...
        % KF: English grammar: that is never preceded by a comma (which is)
        ...
        % KF: modules definitions -> module definitions (please check those plural rules in English)
        ...
        % KF: where KOs contributed by the donor are shown in blue-green whereas those coming from the beneficiary genome are coloured in rose.
        ...
        % KF: more than one [...] genome, not more than one [...] genomes; genomes' combinations -> genome combinations
    ...
    \label{subsec:seeds}
        ...
        % KF: get their overlap -> compute their overlap; pair-wised -> pairwise
...
\label{sec:results}
    ...
    \label{subsec:microbetagdb}
        ...
        % KF: could be represented in a table
