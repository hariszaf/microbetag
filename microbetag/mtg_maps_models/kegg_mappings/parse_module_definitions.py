            ...
            for index, alt in enumerate(indep_alts):
                ...
                if "(" not in alt[0]:
                    ...
                    for k in range(len(parts)):
                        ...
                        inner_parts = [
                            parts[k][i:j]
                            for i, j in zip(inner_indices, inner_indices[1:] + [None])
                        ]
                        ...
                        if len(inner_parts) > 0:
                            ...
                            for inner_part_index in range(len(inner_parts)):
                                ...
                                coord = split_to_independent_chunks(
                                    inner_parts[inner_part_index][0]
                                )

                                if len(coord) == 0:
                                    ready_to_go.append(
                                        inner_parts[inner_part_index][0].split("+")
                                    )
                                ...
                                # Thus, we only deal with this for now so please always check for this warning message in case a new module goes throug this case
                                    ...
                                    last_parts = [
                                        inner_parts[inner_part_index][0][i:j]
                                        for i, j in zip(coord, coord[1:] + [None])
                                    ]
                                    last_parts = [x for x in last_parts if x]
                                    last_parts = [
                                        re.split(r"\+|,|_", x) for x in last_parts
                                    ]
                            ...
                            inner_parts = [
                                inner_parts[j]
                                for j in range(len(inner_parts))
                                if inner_parts[j]
                            ]
                            inner_parts = [
                                inner_parts[u].split("+")
                                for u in range(len(inner_parts))
                            ]
                            inner_parts = [list(flatten(inner_parts))]
                            inner_parts = [[x for x in inner_parts[0] if x]]
...
module_steps_parsed = parse_regular_module_dictionary(
    module_components_raw, structurals
)
